...
public final class AssetCommandProvider implements CommandProvider {
	...
	/**
	 * The command {@code addOpcUaChannel} creates an OPC-UA channel and
	 * associates it with the provided asset name
	 *
	 * @param ci
	 *            the console interpreter
	 ...
	 *             while updating
	 */
	public void _addOpcUaChannel(final CommandInterpreter ci) throws KuraException {
		final String argument = ci.nextArgument();
		if (argument == null) {
			ci.println(
					"Usage: addOpcUaChannel <assetPid>#<channelName>#<type>#<valueType>#<node.id>#<node.namespace.index> - Creates an OPC-UA Channel");
			return;
		}
		final List<String> prop = Arrays.asList(argument.split("#"));
		final Map<String, Object> map = CollectionUtil.newHashMap();
		...
		map.put(i + ".CH.type", prop.get(2));
		map.put(i + ".CH.value.type", prop.get(3));
		map.put(i + ".CH.DRIVER.node.id", String.valueOf(prop.get(4)));
		map.put(i + ".CH.DRIVER.node.namespace.index", Integer.valueOf(prop.get(5)));
		this.m_configService.updateConfiguration(prop.get(0), map);
	}
	...
	public String getHelp() {
		return "---Wire Asset Service---\n"
				...
				+ "\taddOpcUaChannel <assetPid>#<channelName>#<type>#<valueType>#<node.id>#<node.namespace.index> - Creates an OPC-UA Channel";
	}
