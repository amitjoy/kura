...
public final class ModbusDriver implements Driver {
    ...
    public void connect() throws ConnectionException {
        logger.debug(message.connectingModbus());
        try {
            this.modbusMaster.connect();
        } catch (final Exception e) {
            logger.error(message.connectionProblem(), e);
            throw new ConnectionException(message.connectionProblem(), e);
        }
        logger.debug(message.connectingModbusDone());
    ...
    public void disconnect() throws ConnectionException {
        ...
        if (this.isConnected) {
            logger.debug(message.disconnectingModbus());
            try {
            ...
            logger.debug(message.disconnectingModbusDone());
    ...
    private int getReadFunctionCode(final String primaryTable) {
        requireNonNull(primaryTable, message.primaryTableNonNull());
    ...
    private TypedValue<?> getValue(final Object response, final DriverRecord record) {
        requireNonNull(response, message.responseNonNull());
        ...
        for (final DriverRecord record : records) {
            ...
            if (!channelConfig.containsKey(UNIT_ID)) {
                record.setDriverStatus(
                        new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE, message.errorRetrievingUnitId(), null));
                record.setTimestamp(System.currentTimeMillis());
                continue;
            }
            ...
            if (!channelConfig.containsKey(PRIMARY_TABLE)) {
                record.setDriverStatus(new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE,
                        message.errorRetrievingPrimaryTable(), null));
                record.setTimestamp(System.currentTimeMillis());
                continue;
            }
            ...
            if (!channelConfig.containsKey(MEMORY_ADDRESS)) {
                record.setDriverStatus(new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE,
                        message.errorRetrievingMemAddr(), null));
                record.setTimestamp(System.currentTimeMillis());
                continue;
            }
            int unitId;
            ...
            if (functionCode == 0) {
                record.setDriverStatus(new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE,
                        message.errorRetrievingFunctionCode(), null));
                record.setTimestamp(System.currentTimeMillis());
                continue;
            }
            ...
            if ((unitId < 1) || (unitId > 247)) {
                record.setDriverStatus(
                        new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE, message.wrongUnitId(), null));
                record.setTimestamp(System.currentTimeMillis());
                continue;
            }
            ...
            if ((memoryAddr < 0) || (memoryAddr > 65535)) {
                record.setDriverStatus(
                        new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE, message.wrongRegister(), null));
                record.setTimestamp(System.currentTimeMillis());
                continue;
            }
            try {
                ...
                record.setDriverStatus(
                        new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE, message.connectorNonNull() + " OR "
                                + message.wrongUnitId() + " OR " + message.wrongValueType(), e));
            }
            record.setTimestamp(System.currentTimeMillis());
        }
        this.disconnect();
    ...
    private synchronized Object readRequest(final Map<String, Object> channelConfig, final int unitId,
            ...
            throws ModbusException {
        requireNonNull(channelConfig, message.channelConfigNonNull());
        requireNonNull(modbusMaster, message.connectorNonNull());
        ...
        default:
            throw new ModbusException(message.requestTypeNotSupported(functionCode));
        }
    }
    ...
    public List<DriverRecord> write(final List<DriverRecord> records) throws ConnectionException {
        ...
        for (final DriverRecord record : records) {
            ...
            try {
                ...
                record.setDriverStatus(new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE,
                        message.connectorNonNull() + " OR " + message.wrongUnitId(), e));
            } catch (final NumberFormatException nfe) {
                record.setDriverStatus(new DriverStatus(DRIVER_ERROR_CHANNEL_NOT_ACCESSIBLE,
                        message.errorRetrievingValueType(), nfe));
            }
            record.setTimestamp(System.currentTimeMillis());
        }
        this.disconnect();
